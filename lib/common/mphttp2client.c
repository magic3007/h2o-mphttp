...
size_t h2o_mpclient_get_bw(h2o_mphttp2client_t *mp){
    h2o_mpclient_update_bw(mp);
    fprintf(stdout, "mpclient(%d): bandwidth = %zu KB/s\n", mp->ID, mp->bandwith > 0 ? mp->bandwith / 1024: 128);
    fflush(stdout);
    if(mp->bandwith > 0)
        return mp->bandwith;
...
int h2o_mpclient_reschedule(h2o_mphttp2client_t *mp_idle){
    ...
    if((remain = h2o_rangeclient_get_remain(ra_busy)) <= 64 * 1024 /* 64KB */)
        return 0;
